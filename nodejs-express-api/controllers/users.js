...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		query.order = DB.getOrderBy(req, 'u_id', 'desc');
...
router.post('/edit/:recid', 
	[
		...
		body('telephone').optional({nullable: true, checkFalsy: true}),
		body('u_id_fk').optional({nullable: true, checkFalsy: true}),
...
, async (req, res) => {
	try{
		...
		// check if username already exist.
		let usernameCount = await DB.Users.count({where:{'username': modeldata.username, 'u_id': {[DB.op.ne]: recid} }});
		if(usernameCount > 0){
			return res.badRequest(`${modeldata.username} already exist.`);
		}
		const query = {};
